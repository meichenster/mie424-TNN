...
"""
This is a CP model that aims to find the BNNs with the maximal sum of the neuron's margins that fits the training set.
"""
...
class MultiLayerPerceptron:
    def __init__(self, layers, data, labels):
        ...
        # This variable keeps expressions all the margins per neuron
        self.margins = {}
        for layer_id in range(1,len(self.layers)):
            for neuron_out in range(self.layers[layer_id]):
    ...
    def add_example(self, data, label, show = False):
        ...
        for layer_id in range(1, n_layers):
            ...
            for n_out in range(h_size):
                ...
                self.margins[(layer_id, n_out)].append(abs(pre_activation))
    ...
    def train(self, n_threads, time_out):
        ...
        # Setting the objective function (maximize the margin)
        sum_margins = sum([min(m) for m in self.margins.values()])
